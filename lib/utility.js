...
const createMessageIdlogger = getLogger('utility.createMessageId')
...
function createMessageId() {
  ...
  createMessageIdlogger.debug('MessageId(%s) was created with time = %s, lower = %s,  messageID binary = %s',
...
const createSHAHashlogger = getLogger('utility.createSHA1Hash')
function createSHAHash(buffer, algorithm) {
  const sha1sum = crypto.createHash(algorithm || 'sha1')
  if (util.isArray(buffer)) {
    createSHAHashlogger.debug('It\'s an Array of buffers')
    buffer.forEach(b => sha1sum.update(b))
  } else {
    createSHAHashlogger.debug('It\'s only one buffer')
...
const modPowlogger = getLogger('utility.aesDecrypt')
// Mod Pow
function modPow(x, e, m) {
  ...
  modPowlogger.debug('X = %s, E = %s, M = %s, result = %s', bigX, bigE, bigM, bigResult)
