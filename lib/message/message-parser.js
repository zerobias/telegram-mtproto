...
MessageParser.prototype.parse = function (message, duration) {
    switch (message._typeName) {
        ...
        case 'mtproto.type.Msgs_ack':
            ...
            this.emit(message._typeName, message.msg_ids.list);
            break;
        default :
            if (logger.isDebugEnabled()) {
                logger.debug('%s found', message._typeName, duration);
            }
            this.emit(message._typeName, message, duration);
    }
};
