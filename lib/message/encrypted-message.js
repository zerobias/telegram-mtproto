    ...
    if (opts.message) {
        if (util.isArray(opts.message)) {
            ...
            for (var i = 0; i < opts.message.length; i++) {
                ...
                var isContentRelated = (message.typeName.slice(0, 8) != 'mtproto.' || message.typeName.slice(-15) == 'invokeWithLayer') ;
                if (logger.isDebugEnabled()) {
                    logger.debug('Message [%s] is content related? %s', message.typeName, isContentRelated);
                }
