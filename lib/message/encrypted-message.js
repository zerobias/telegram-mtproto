    ...
    if (opts.message) {
        if (util.isArray(opts.message)) {
            ...
            for (var i = 0; i < opts.message.length; i++) {
                ...
                var isContentRelated = (message._typeName.slice(0, 8) != 'mtproto.' || message._typeName.slice(-15) == 'invokeWithLayer') ;
                if (logger.isDebugEnabled()) {
                    logger.debug('Message [%s] is content related? %s', message._typeName, isContentRelated);
                }
