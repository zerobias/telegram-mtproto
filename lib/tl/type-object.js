...
TypeObject.prototype.writeBytes = function(bytes, useWordLength) {
  ...
  const realLength = typeof bytes == 'string' ? unescape(encodeURIComponent(bytes)).length : bytes.length
  const bLength = useWordLength ? realLength / 4 : realLength
  ...
  if (!useWordLength) {
    const padding = (offset + realLength) % 4
    if (padding > 0) {
