...
//  - publicKey
//  - messageBuffer
function rsaEncrypt(publicKey, messageBuffer) {
    ...
    if (messageLength < 255) {
        ...
        messageBuffer = Buffer.concat([messageBuffer, util.createRandomBuffer(paddingLength)]);
    }
...
function aesDecrypt(msg, key, iv) {
    ...
    if (logger.isDebugEnabled()) logger.debug('encryptedMsg = %s\nkeyWordArray = %s\nivWordArray = %s',
...
function aesEncrypt(msg, key, iv) {
    ...
    if (padding > 0) {
        paddingBuffer = util.createRandomBuffer(16 - padding);
    ...
    var keyWordArray = buffer2WordArray(key);
    var ivWordArray = buffer2WordArray(iv);
    if (logger.isDebugEnabled()) logger.debug('plainMsg = %s\nkeyWordArray = %s\nivWordArray = %s',
