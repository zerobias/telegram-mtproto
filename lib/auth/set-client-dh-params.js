...
function setClientDHParams(callback, context) {
  mtproto.service.set_client_DH_params({
    ...
    callback: function(ex, setter) {
      if (ex) {
        logger.error(ex)
        if (callback) callback(ex)
      } else {
        const instof = typeName => setter.instanceOf(typeName)
        switch (true) {
          ...
          case instof('mtproto.type.Dh_gen_retry'): {
            ...
            callback(createError(JSON.stringify(setter), 'EDHPARAMRETRY'))
            break
          }
          case instof('mtproto.type.Dh_gen_fail'): {
            logger.warn('\'Dh_gen_fail\' received from Telegram!')
            callback(createError(JSON.stringify(setter), 'EDHPARAMFAIL'))
            break
          }
          default: {
