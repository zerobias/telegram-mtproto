...
function timeSynchronization(serverTime, requestDuration) {
    ...
    logger.info('ServerTime %ss - LocalTime %ss - Response in %sms',
        serverTime, localTime, response);
    if (lastResponse > response) {
        lastResponse = response;
        response = Math.floor(response/1000);
        timeOffset = ((serverTime + response) - localTime);
        logger.info('time-synchronization: (ServerTime %ss + server-response %ss) - LocalTime %ss = timeOffset %ss',
            serverTime, response, localTime, timeOffset);
    } else {
        logger.info('NO time-synchronization: the server-response (%s) took more time than the previous one (%s)',
