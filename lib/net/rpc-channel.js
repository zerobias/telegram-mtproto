...
RpcChannel.prototype._call = function(reqMsg, context, callback) {
        ...
        conn.read(function(ex2, response) {
            ...
            try {
                if (logger.isDebugEnabled()) {
                    logger.debug('response: %s ', response.toString('hex'));
                }
                var resMsg = self._deserializeResponse(response, context);
                var Type = TypeBuilder.requireTypeFromBuffer(resMsg);
                var resObj = new Type({buffer: resMsg});
                resObj.deserialize();
                var duration = new Date().getTime() - start;
                if (logger.isDebugEnabled()) {
                    logger.debug('%s executed in %sms', context.methodName, duration);
                }
                if (callback) {
                    callback(null, resObj, duration);
                }
            } catch (ex3) {
                logger.error('Unable to deserialize response %s from %s due to %s ', response.toString('hex'), context.methodName, ex3);
                if (callback) {
                    callback(ex3);
                }
            }
