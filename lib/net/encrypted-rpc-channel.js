...
class EncryptedRpcChannel extends RpcChannel {
  ...
  _onRpcResult(rpcResult, duration) {
    ...
    if (callback) {
      ...
      const pause = new Promise(rs => setTimeout(rs, 1e3))
  ...
  callMethod(method, callback = defCall) {
    const caller = (rs, rj) => {
      const cb = recall(data => rs(removeFuncs(data)), rj)
      if (!this._open) {
...
const typeOf = obj => {
  switch (true) {
    ...
    case Array.isArray(obj)       : return 'array'
    case typeof obj === 'function': return 'function'
    case typeof obj === 'object'  : return 'object'
    default                       : return 'default'
  }
}

const removeFuncs = message => {
  if (message) {
    if (message.logger)
      removeKey(message, 'logger')
    for (const key of Object.keys(message)) {
      switch (typeOf(message[key])) {
        case 'null': break
        case 'array': message[key] = message[key].map(removeFuncs); break
        case 'function': removeKey(message, key); break
        case 'object': removeFuncs(message[key]); break
      }
    }
  }
  return message
